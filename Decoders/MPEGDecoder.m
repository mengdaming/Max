...
{	
    ...
    UInt32				frameCount			= (UInt32)([buffer freeSpaceAvailable] / (_pcmFormat.mChannelsPerFrame * sizeof(int16_t)));
    ...
    for(;;) {
        ...
        if(NULL == _mad_stream.buffer || MAD_ERROR_BUFLEN == _mad_stream.error) {
            if(NULL != _mad_stream.next_frame) {
                bytesRemaining = (UInt32)(_mad_stream.bufend - _mad_stream.next_frame);
        ...
        if(-1 == result) {
            if(MAD_RECOVERABLE(_mad_stream.error)) {
                ...
                const uint8_t	*buffer1			= _mad_stream.this_frame;
                unsigned		buflen			= (unsigned int)(_mad_stream.bufend - _mad_stream.this_frame);
                uint32_t		id3_length		= 0;
                
                if(10 <= buflen && 0x49 == buffer1[0] && 0x44 == buffer1[1] && 0x33 == buffer1[2]) {
                    id3_length = (((buffer1[6] & 0x7F) << (3 * 7)) | ((buffer1[7] & 0x7F) << (2 * 7)) |
                                  ((buffer1[8] & 0x7F) << (1 * 7)) | ((buffer1[9] & 0x7F) << (0 * 7)));
        ...
        if(_foundLAMEHeader && [self totalFrames] < _samplesDecoded + (sampleCount - startingSample))
            sampleCount = (unsigned int)([self totalFrames] - _samplesDecoded);
    ...
    for(;;) {
        if(NULL == stream.buffer || MAD_ERROR_BUFLEN == stream.error) {
            if(stream.next_frame) {
                bytesRemaining = (UInt32)(stream.bufend - stream.next_frame);
    ...
    for(;;) {
        ...
        if(-1 == result) {
            if(MAD_RECOVERABLE(_mad_stream.error)) {
                ...
                unsigned		buflen			= (unsigned)(_mad_stream.bufend - _mad_stream.this_frame);
                uint32_t		id3_length		= 0;
